...
class admin_forum extends base_forum {
  ...
  */
   ...
   * @var array
   */
  private $categsOpen = array();

  /**
   * @var base_dialog
   */
  private $categDialog = NULL;

  /**
   * @var base_dialog
   */
  private $forumDialog = NULL;


  /**
   * @var base_dialog
   */
  private $entryDialog = NULL;
  ...
  function getXMLSurfersTree() {
    ...
    foreach ($surfers as $surferId => $surferData) {
      ...
      $result .= sprintf(
        ...
        papaya_strings::escapeHTMLChars($images['items-user']),
        $selected
      );
    }
    $result .= '</items>'.LF;
  ...
  */
  function getXMLEntry($entry, $threadId, $indent = 0) {
    ...
    $result .= sprintf(
      ...
      papaya_strings::escapeHTMLChars($images['items-message']),
      (int)$indent,
      $selected
    );
    $result .= sprintf(
  ...
  function getXMLThreadElement($id, $indent) {
    ...
    if ($entry['entry_thread_count'] > 0) {
      if (isset($this->entryTree[$id])) {
        foreach ($this->entryTree[$id] as $childId) {
  ...
  */
  ...
  function pasteForum ($forumId, $categId) {
    ...
    return FALSE !== $this->databaseUpdateRecord(
      $this->tableBoards, $data, 'forum_id', (int)$forumId
    );
  }
  ...
  function getXMLLastEntries() {
    if (isset($this->entries) && is_array($this->entries) && count($this->entries) > 0) {
      ...
      foreach ($this->entries as $entryId => $entry) {
        ...
        $icon = $images['items-message'];

        // determine state of post
        if ($entry['entry_blocked'] > 0) {
          $icon = $images['status-user-evil'];
        }
        ...
        if ($entry['entry_blocked'] > 0) {
          ...
          $blockIcon = $images['status-user-angel'];
          $blockHint = 'Unblock entry';
        } else {
          $blockCommand = 'block_entry';
          $blockIcon = $images['status-user-evil'];
  ...
  */
  function getCommunityObject() {
    if (!(isset($this->_communityObject) && is_object($this->_communityObject))) {
      $this->_communityObject = $this->papaya()->plugins->get(
  ...
  */
  function generatePagingLinksXml(
    $paramName = 'offset', $baseParams = array(), $offset = 0, $limit = 20, $count = 0
  ) {
  ...
  */
  function initializePagingLimits() {
    ...
    $this->_boardPagingLimit = $options->get('BOARD_LIMIT', 10);
    $this->_surferPagingLimit = $options->get('SURFER_LIMIT', 10);
    $this->_threadPagingLimit = $options->get('THREAD_LIMIT', 10);
  }
