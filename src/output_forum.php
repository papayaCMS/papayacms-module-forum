...
* @version $Id: output_forum.php 39732 2014-04-08 15:34:45Z weinert $
*/
...
class output_forum extends base_forum {
  ...
  */
   ...
   * @var base_dialog
   */
  public $outputDialog = NULL;

  /**
   * @var base_dialog
   */
  public $searchDialog = NULL;
  ...
  */
   ...
   * @var array
   */
  protected $_baseParameters = array();
  ...
  */
   ...
   *
   * @internal param bool $root optional, default FALSE
   * @return string
   */
  protected function _getOutputOverview() {
    ...
    if (!empty($this->categs) && is_array($this->categs)) {
      $innerCategoriesXml = '';
      foreach ($this->categs as $categData) {
   ...
   * @return string
   */
  protected function _getOutputForum() {
    ...
    if ($threadId != 0) {
      ...
      if (!empty($this->entries) && is_array($this->entries)) {
        if (count($this->entries) < $this->spliceOfs) {
          ...
          $page->sendHeader('X-Papaya-Status: forum entry offset out of bounds');
          $page->sendHeader('Location: '.$this->_getLinkWithParams($params, NULL, TRUE));
          exit();
        }
        if ($this->spliceOfs / $this->spliceMax != (int)($this->spliceOfs / $this->spliceMax)) {
          ...
          $page->sendHeader('X-Papaya-Status: forum entry offset in between pages');
          $page->sendHeader('Location: '.$this->_getLinkWithParams($params, NULL, TRUE));
          exit();
        }
  ...
  */
   ...
   * @return string $result xml
   */
  protected function _getLinksXml($links = array()) {
    $result = '';
  ...
  */
  function sendNotificationMail($email, $name, $type) {
    ...
    $data = array(
      'PROJECT' => $this->papaya()->options->get('PAPAYA_PROJECT_TITLE', ''),
      'NOTIFY' => $type,
      'RECEIVER' => $name,
